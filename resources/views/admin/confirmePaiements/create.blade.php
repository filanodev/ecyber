        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.confirmePaiement.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.confirme-paiements.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('mode_paiement') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="mode_paiement_id" id="mode_paiement_id">
                                @foreach($mode_paiements as $id => $entry)
                                    <option value="{{ $id }}" {{ old('mode_paiement_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('payement_ref') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="payement_ref" id="payement_ref" value="{{ old('payement_ref', '') }}">
                        ...
                        <div class="form-group {{ $errors->has('montant') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="montant" id="montant" value="{{ old('montant', '') }}" step="0.01">
                        ...
                        <div class="form-group {{ $errors->has('compte_payement') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="compte_payement" id="compte_payement" value="{{ old('compte_payement', '') }}">
                        ...
                        <div class="form-group {{ $errors->has('datetime') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="datetime" id="datetime" value="{{ old('datetime', '') }}">

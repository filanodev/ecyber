                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.prices.update", [$price->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('router') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="router_id" id="router_id" required>
                                @foreach($routers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('router_id') ? old('router_id') : $price->router->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('libelle') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="libelle" id="libelle" value="{{ old('libelle', $price->libelle) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('price_sell') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="price_sell" id="price_sell" value="{{ old('price_sell', $price->price_sell) }}" step="0.01" required>
                        ...
                        <div class="form-group {{ $errors->has('users') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="users_id" id="users_id" required>
                                @foreach($users as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('users_id') ? old('users_id') : $price->users->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('users'))
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status" id="status">
                                ...
                                @foreach(App\Models\Price::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $price->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>

        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.modePaiement.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.mode-paiements.update", [$modePaiement->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('countries') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="countries[]" id="countries" multiple>
                                @foreach($countries as $id => $country)
                                    <option value="{{ $id }}" {{ (in_array($id, old('countries', [])) || $modePaiement->countries->contains($id)) ? 'selected' : '' }}>{{ $country }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('slog') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="slog" id="slog" value="{{ old('slog', $modePaiement->slog) }}">
                            @if($errors->has('slog'))
                        ...
                        <div class="form-group {{ $errors->has('libelle') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="libelle" id="libelle" value="{{ old('libelle', $modePaiement->libelle) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="description" id="description">{!! old('description', $modePaiement->description) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status" id="status">
                                ...
                                @foreach(App\Models\ModePaiement::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $modePaiement->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('api_key') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="api_key" id="api_key" value="1" {{ $modePaiement->api_key || old('api_key', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('site_token') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="site_token" id="site_token" value="1" {{ $modePaiement->site_token || old('site_token', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('notify_url') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="notify_url" id="notify_url" value="1" {{ $modePaiement->notify_url || old('notify_url', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('return_url') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="return_url" id="return_url" value="1" {{ $modePaiement->return_url || old('return_url', 0) === 1 ? 'checked' : '' }}>
